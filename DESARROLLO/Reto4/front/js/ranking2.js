document.addEventListener('DOMContentLoaded', async () => {
    try {
      const response = await fetch('http://localhost:3030/ahorcado/obtenerRanking');
      if (response.ok) {
    ...
    async function mostrarRanking(rankingData) {
        const rankingContainer = document.getElementById('ranking-container2'); // Reemplaza con el ID o selector correcto de tu contenedor de ranking
    ...
    async function obtenerNombreUsuario(usuarioId) {
        try {
          const response = await fetch(`http://localhost:3030/ahorcado/obtenerNombre/${usuarioId}`);
          if (response.ok) {
